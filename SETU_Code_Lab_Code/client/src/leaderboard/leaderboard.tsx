...
export default function Leaderboard() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      const res = await api.get(
        `/api/fetchLeaderboard?dateRange=${dateRange}&filterBy=${filterBy}&courseId=${selectedCourse}`,
      );
      if (res.ok) {
  ...
  return (
    <div>
      ...
      {(coursesLoading || leaderboardLoading) && (
        <div className="spinner">
          <FadeLoader color="#dedede" />
          <p style={{ margin: 24 }}>Hang tight, Loading Leaderboard...</p>
        </div>
      )}
    </div>
  );
