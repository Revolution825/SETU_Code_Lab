  ...
  useEffect(() => {
    async function fetchAllData() {
      try {
        ...
        if (course?.course_id) {
          const assocRes = await api.post("/api/problemStudentId", {
            course_id: JSON.stringify(course.course_id),
          });
          if (assocRes.ok) {
            const assocData = await assocRes.json();

            const problemIds =
              assocData.problemIds?.map(
                (p: { problem_id: number }) => p.problem_id,
              ) ?? [];
            const studentIds =
              assocData.studentIds?.map(
                (s: { user_id: number }) => s.user_id,
              ) ?? [];

            setSelectedProblems(
              problemsData
                .map((p: { problem_id: number }) => p.problem_id)
                .filter((id: any) => problemIds.includes(id)),
            );

            setSelectedStudents(
              studentsData
                .map((s: { student_id: number }) => s.student_id)
      ...
      <div>
        ...
        >
          <div className="createCourseContent">
            <div className="createCourseFormBackground">
              <div className="createProblemHeader">Create New Course</div>
            ...
            <div className="selectionContainer">
              <div className="selectionBox" style={{ marginRight: "24px" }}>
                <div className="createProblemHeader">Select Problems</div>
                <div className="filteringOptions">
                  <img
                    src="search.svg"
                    alt="search icon"
                    className="createClassSearchIcon"
                  />
                  <input
              ...
              <div className="selectionBox">
                <div className="createProblemHeader">Select Students</div>
                <div className="filteringOptions">
                  <img
                    src="search.svg"
                    alt="search icon"
            ...
            <div className="submitButtonDiv">
              <button type="submit" className="submitCourseButton">
                Save
              </button>
            </div>
          </div>
        ...
        {dataLoading && (
          <div className="spinner">
            ...
            <p style={{ margin: 24 }}>Hang tight, Loading Course Details...</p>
          </div>
        )}
      </div>
    </>
