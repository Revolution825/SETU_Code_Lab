...
export const fetchStudentsOnCourse = async (course_id: number) => {
  const result = await pool.query(
    "SELECT u.user_id, u.user_name FROM users u JOIN enrollment e ON u.user_id = e.user_id WHERE e.course_id = $1 AND u.role = 'student'",
    [course_id],
  );
  return result.rows;
};
...
export const fetchUserData = async (user_id: number) => {
  const result = await pool.query(
    "SELECT user_id, user_name, email, role, total_points, current_streak, longest_streak, last_solved_date FROM users WHERE user_id = $1",
    [user_id],
  );
...
export const updateStreak = async (
  ...
  await client.query(
    `UPDATE users SET current_streak=$1, longest_streak=GREATEST(longest_streak, $1), last_solved_date=CURRENT_DATE WHERE user_id=$2`,
    [currentStreak, user_id],
  );
};
